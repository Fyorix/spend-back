...
describe('GeocodingService (Unit)', () => {
  ...
  beforeEach(() => {
    ...
    googleProvider = internal.providers[GeocodingProviderType.GOOGLE_MAPS] as GoogleMapsProvider;
  ...
  it('should return coordinates from Google Maps when specified', async () => {
    ...
    const googleSpy = jest.spyOn(googleProvider, 'geocode').mockResolvedValue(coordinate);

    const result = await service.geocode(address, GeocodingProviderType.GOOGLE_MAPS);
  ...
  it('should return coordinates from OpenStreetMap when specified', async () => {
    const address = 'Paris';
    const coordinate = createCoordinate();
    const osmSpy = jest.spyOn(osmProvider, 'geocode').mockResolvedValue(coordinate);

    const result = await service.geocode(address, GeocodingProviderType.OPEN_STREET_MAP);
  ...
  it('should return null if the provider fails', async () => {
    ...
    const googleSpy = jest.spyOn(googleProvider, 'geocode').mockResolvedValue(null);
  ...
  it('should catch errors from providers and return null', async () => {
    const address = 'Error';
    jest.spyOn(googleProvider, 'geocode').mockRejectedValue(new Error('API Down'));
