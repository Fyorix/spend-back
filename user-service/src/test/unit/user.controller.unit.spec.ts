...
describe('UserController', () => {
    ...
    describe('getUser', () => {
        ...
        it('should throw an error if the user is not found', async () => {
            ...
            jest.spyOn(userService, 'getUserById').mockRejectedValue(new UserNotFoundException('User not found'));

            await expect(userController.getUser(userId)).rejects.toThrow(UserNotFoundException);
    ...
    describe('register', () => {
        ...
        it('should throw an error if the user already exists', async() => {
            ...
            jest.spyOn(userService, 'register').mockRejectedValue(new UserAlreadyExistsException('User already exists'));

            await expect(userController.register(userConnectDto)).rejects.toThrow(UserAlreadyExistsException);
            expect(userService.register).toHaveBeenCalledWith(userConnectDto);
        });
    ...
    describe('deleteUser', () => {
        ...
        it('should throw an error if the user is not found', async() => {
            const userId = 'non-existent-id';
            jest.spyOn(userService, 'deleteUserById').mockRejectedValue(new UserNotFoundException('User not found'));

            await expect(userController.deleteUser(userId)).rejects.toThrow(UserNotFoundException);
    ...
    describe('login', () => {
        ...
        it('should throw an error if the credentials are invalid', async() => {
            ...
            jest.spyOn(authService, 'login').mockRejectedValue(new InvalidCredentialsException('Invalid credentials'));

            expect(userController.login(userConnectDto)).rejects.toThrow(InvalidCredentialsException);
            expect(authService.login).toHaveBeenCalledWith(userConnectDto);
        });
    });
});